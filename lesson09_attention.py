...
class block(torch.nn.Module):
    ...
    def forward(self, x): # x:(batches_num, tokens_num, hidden_size)
        ...
        mask = torch.tril(torch.ones(tokens_num, tokens_num)) # 生成一个下三角形矩阵
...
wte = torch.nn.Embedding(vocab_size, hidden_size) # 词嵌入表
wpe = torch.nn.Embedding(1024, hidden_size) # 位置表
params = list(wte.parameters()) + list(wpe.parameters()) #收集参数

blocks = [block(hidden_size, head_num) for _ in range(6)]
...
lm_head = torch.nn.Linear(hidden_size, vocab_size) # 生成每个字符的概率
...
idxs = torch.tensor([data_input]) # 输入字符 (batches_num, token_num)
pos = torch.arange(len(data_input)).unsqueeze(0) # 位置输入 (batches_num, token_num)
targets = torch.tensor([exp]) # 预期值
